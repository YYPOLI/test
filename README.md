...
│   ├── spender_history/             # Pre-built: spender tx history (139 of 175 addresses)
│   └── pipeline_output/             # Generated by pipeline scripts (gitignored)
│       ├── raw_traces/              #   BigQuery JSON exports
│       ├── cleaned/                 #   Cleaned & parsed CSVs
│       ├── spender_history/         #   Full Etherscan spender tx history (24k+ dirs)
...
> **Note on spender history data:** The repository includes pre-built spender transaction histories for 139 addresses under `data/spender_history/`. However, 36 high-volume addresses (Uniswap Permit2, Aave, Lido, Ether.fi, Puffer Finance, etc.) are excluded due to their combined size (~3.2 GB). These are well-known benign protocol contracts with no malicious activity. See `data/spender_history/EXCLUDED_LARGE_ADDRESSES.md` for the full list. To crawl the missing histories:
>
> ```bash
> python pipeline/data_processing/spender_history_crawler.py
> ```
>
> An `ETHERSCAN_API_KEY` in `.env` is required (free tier: 5 calls/sec).
