...
from data.02_data_processing.data_processor import get_file_list
...
def _load_knowledge_base():
    ...
    sp_df = pd.read_csv(os.path.join(analysis_dir, "spender_analysis_result_3months_0209.csv"))
    sp_df['address'] = sp_df['address'].str.lower().str.strip()
    sp_map = sp_df.set_index('address').to_dict(orient='index')

    sb_df = pd.read_csv(os.path.join(analysis_dir, "original_submitter_analysis_result_3months_0211.csv"))
    ...
    tf_df = pd.read_csv(os.path.join(analysis_dir, "transfer_analysis_result_3months_0209.csv"))
    tf_df['tx_hash'] = tf_df['tx_hash'].str.lower().str.strip()

    tk_path = os.path.join(BASE_PATH, "verified_permit/labeled_address/features/Top_1000_Tokens_Decimals.csv")
    tk_df = pd.read_csv(tk_path)
...
def compute_interaction_features():
    ...
    FILE_PATH = os.path.join(BQ_PATH, "cleaned_data/test_data/")
    ...
    out_file = os.path.join(BASE_PATH, "BigQuery_since20251230/analysis/interaction_features_result.csv")
...
def analyze_interaction_features():
    ...
    FEATURE_PATH = os.path.join(BASE_PATH, "BigQuery_since20251230/analysis/interaction_features_result_3months_0211.csv")
